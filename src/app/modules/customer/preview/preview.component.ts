...
export class PreviewCustomerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.routeInfo.params.subscribe( param => {
      ...
      /* ------------------- 查看跟进记录 ------------------- */
      this.http.post('/retrunVisit/showFollowRecord', {id: this._id }, false).then(res => {
        if (res.code == 1000) {
    ...
    this.recordFormModel = this.fb.group({
      ...
      staffId       : [null, [].concat(this.router.url.indexOf('/visit/member') > -1 ? [] : [Validators.required])],                                // 跟进老师
      ...
      reserve       : this.fb.group({
                        ...
                        swimTeacherId : [null, [].concat(this.router.url.indexOf('/visit/member') > -1 ? [] : [Validators.required])]
                    })
    });
    ...
    this._updateFollowRecordFormModel = this.fb.group({
      ...
      staffId       : [, [].concat(this.router.url.indexOf('/visit/member') > -1 ? [] : [Validators.required])],                                // 跟进老师
      ...
      reserve       : this.fb.group({
                        ...
                        swimTeacherId : [, [].concat(this.router.url.indexOf('/visit/member') > -1 ? [] : [Validators.required])]
                    })                                          
    });
  ...
  intentionCustomer(): void {
    this.http.post('/retrunVisit/transitioNoIntention', { id: this._id }).then( res => {
  ...
  saveUpdateFollowRecord(): void {
    ...
    if (mainValid && childValid) {
      ...
      delete params.reserve;
      this.http.post('/retrunVisit/modifyClubFollowRecord', { paramJson: JSON.stringify(params) }, false).then(res => {
