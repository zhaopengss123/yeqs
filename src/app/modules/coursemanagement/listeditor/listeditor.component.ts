...
export class ListeditorComponent implements OnInit {
  ...
  selectDataArr(){
   //查询课程类别 
    this.http.post('/intelligent/selectLessonAll', { status: '1' }, false).then(res => {
      if (res.code == 1000) {
    ...
    //查询课程类型
    this.http.post('/intelligent/selectLessonAll', { status: '0' }, false).then(res => {
      if (res.code == 1000) {
  ...
  querySelect(){
    ...
    this.http.post('/scheduling/selectCondition', { paramJson }, false).then(res => {
      if (res.code == 1000) {
  ...
  EditingCourse(id){
    ...
    this.addcourse = true;
    this.courseId = id;
    this.http.post('/scheduling/selectSyllabus', { id:id }, false).then(res => {
      if (res.code == 1000) {
...
//删除弹框确认按钮
  isremoveanddisable(){
    ...
    if (this.openType==0){
      this.http.post('/scheduling/deleteSyllabus', { syllabusName: this.coursesName }, false).then(res => {
        if (res.code == 1000) {
  ...
  courseContent(id){
    ...
    this.ListmainId = id;
    this.http.post('/scheduling/selectContentsAll', { id: id }, false).then(res => {
      if (res.code == 1000) {
...
//新增课程内容
  ...
  isremoveclassContent(){
    this.http.post('/scheduling/deleteContents', { id: this.classListid }, false).then(res => {
      if (res.code == 1000) {
        this.removeclassContent = false;
        this.message.create('success', '删除成功！');
        this.courseContent(this.ListmainId);
  ...
  modifyListContent(id){
    this.http.post('/scheduling/selectContents', { id: id }, false).then(res => {
      if (res.code == 1000) {
        this.addclassContent = true;
