...
export class ClassroomComponent implements OnInit {
  ...
  queryDataList() {
    ...
    this.http.post('/scheduling/selectClassRoom', { paramJson }, false).then(res => {
      if (res.code == 1000) {
  ...
  selectTeacher() {
    this.http.post('/scheduling/selectEmployee', { name: this.name }, false).then(res => {
      if (res.code == 1000) {
  ...
  isaddclassroom() {
    ...
    if (!this.roomId) {
      ...
      this.http.post('/scheduling/insertScheduling', { paramJson }, false).then(res => {
        if (res.code == 1000) {
      ...
      this.http.post('/scheduling/updateScheduling', { paramJson }, false).then(res => {
        if (res.code == 1000) {
  ...
  Editingclassroom(id) {
    this.addclassroom = true;
    this.http.post('/scheduling/selectScheduling', { id: id }, false).then(res => {
      if (res.code == 1000) {
  ...
  isdelectclassroom() {
    ...
    this.http.post('/scheduling/deleteScheduling', { id: this.roomId }, false).then(res => {
      if (res.code == 1000) {
