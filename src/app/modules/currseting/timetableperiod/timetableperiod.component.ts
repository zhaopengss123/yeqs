...
@Component({
  ...
  styleUrls: ['./timetableperiod.component.scss']
})
export class TimetableperiodComponent implements OnInit {
  ...
  isaddtime() {
    ...
    this.http.post('/intelligent/insertScour', { paramJson }, false).then(res => {
      if (res.code == 1000) {
        this.selectListData();
  ...
  isremoveTime() {
    ...
    this.http.post('/intelligent/deleteScour', { id: id }, false).then(res => {
      if (res.code == 1000) {
        this.selectListData();
